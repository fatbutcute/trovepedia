  ...
  return (
    <div
      ...
      onMouseLeave={() => mouseX.set(Infinity)}
        className={cn(
        "hidden h-16 items-end gap-4 rounded-2xl bg-gray-50 px-4 pb-3 md:flex dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-xl",
        className
        )}>
      {items.map((item) => (
...
function IconContainer({
  ...
  // X-tengely szerinti távolságszámítás
  let distance = useTransform(mouseX, (val) => {
    let bounds = ref.current?.getBoundingClientRect() ?? { left: 0, width: 0 };
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit rounded-md border border-gray-200 bg-gray-100 px-2 py-0.5 whitespace-pre text-neutral-700 shadow-md dark:border-neutral-900 dark:bg-neutral-800 dark:text-white z-50">
              {title}
            </motion.div>
          )}
        </AnimatePresence>
