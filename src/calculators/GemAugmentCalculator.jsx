...
import { gemAugmentCalcContent } from '../components/guides/content/gemAugmentCalc.content.js';
...
export default function GemAugmentCalculator() {
  ...
  const c = gemAugmentCalcContent[langCode] || gemAugmentCalcContent.en;
  ...
  return (
    <div className="react-calc-wrapper" style={{ '--calc-accent': '#00d2ff', '--calc-accent-rgb': '0,210,255' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '22px' }}>
        {[
          { key: 'stat1', label: c.stat1 },
          { key: 'stat2', label: c.stat2 },
          { key: 'stat3', label: c.stat3 }
        ].map(({ key, label }) => (
          <div key={key} className="calc-stat-card">
      ...
      <button className="calc-submit-btn" onClick={calculateAugments}>{c.calculate}</button>

      {results && (
        <div className="calc-result">
          <div className="calc-result-row">
            <span className="label">{c.totalCompletion}</span>
